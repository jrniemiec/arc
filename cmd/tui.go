...
func reportDiverged(cmd *cobra.Command, svc *service.Service) error {
	...
	fmt.Fprint(out, "  To repair:\n\n")
	fmt.Fprintln(out, "    arc sync repair             merge both sides")
	fmt.Fprint(out, "    arc sync repair --dry-run   show the plan, change nothing\n\n")
	fmt.Fprintln(out, "  It fetches first, prints what each side has, and waits for your OK")
	fmt.Fprintln(out, "  before merging. It tags the current state first, and on a conflict it")
	fmt.Fprint(out, "  undoes everything and names the files to fix.\n\n")

	fmt.Fprint(out, "  To do it by hand instead, in the arc data directory:\n\n")
	...
	fmt.Fprintln(out, "  Fetch first, always: every command above reads origin/main, and merging")
	fmt.Fprint(out, "  a stale one pushes back a rejected non-fast-forward.\n\n")
